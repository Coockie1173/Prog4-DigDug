...
public:
    ...
    void RegisterComponentType(dae::GameObject_Barebones* gameObject, const dae::ComponentInstance* componentPtr, unsigned int typeHash);
    void UnregisterComponentType(dae::GameObject_Barebones* gameObject, const dae::ComponentInstance* componentPtr);
    unsigned int GetComponentTypeHash(dae::GameObject_Barebones* gameObject, const dae::ComponentInstance* componentPtr) const;

private:
    std::vector<std::unique_ptr<dae::GameObject_Barebones>> m_objects;
    std::map<dae::GameObject_Barebones*, std::map<const dae::ComponentInstance*, unsigned int>> m_componentTypes;
};
